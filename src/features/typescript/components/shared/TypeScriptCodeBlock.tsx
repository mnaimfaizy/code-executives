...
interface TypeScriptCodeBlockProps {
  code: string;
  title?: string;
...
/** Simple keyword-based syntax coloring for TypeScript/JavaScript code. */
function tokenizeLine(line: string): React.ReactNode[] {
  // Order matters – longer patterns first
  const pattern =
    /(\/\/.*$|\/\*[\s\S]*?\*\/)|('(?:[^'\\]|\\.)*'|"(?:[^"\\]|\\.)*"|`(?:[^`\\]|\\.)*`)|(\b(?:import|export|from|default|as|type|interface|class|extends|implements|abstract|enum|namespace|module|declare|readonly|keyof|typeof|infer|is|in|of|new|return|if|else|switch|case|break|continue|for|while|do|try|catch|finally|throw|async|await|yield|function|const|let|var|this|super|static|get|set|constructor)\b)|(\b(?:string|number|boolean|void|null|undefined|any|never|unknown|object|symbol|bigint)\b)|(\b(?:true|false|null|undefined|NaN|Infinity)\b)|(\b\d[\d_.]*(?:n)?\b)|([{}()[\];,.:?!<>=+\-*/&|^~%@#])/g;
  ...
  while ((match = pattern.exec(line)) !== null) {
    ...
    const [full, comment, str, keyword, builtinType, literal, num, punct] = match;
    let cls = '';
...
const TypeScriptCodeBlock: React.FC<TypeScriptCodeBlockProps> = ({
  code,
  title,
  language = 'typescript',
  highlightLines = [],
...
export default React.memo(TypeScriptCodeBlock);

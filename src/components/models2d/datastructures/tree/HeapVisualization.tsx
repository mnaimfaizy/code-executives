...
const HeapVisualization: React.FC<HeapVisualizationProps> = ({
  ...
  const heapifyUp = useCallback((heapArray: HeapNode[], startIndex: number): HeapNode[] => {
  ...
  const heapifyDown = useCallback((heapArray: HeapNode[], startIndex: number): HeapNode[] => {
    let currentIndex = startIndex;
    ...
    while (true) {
      ...
      // Swap nodes
      [heapArray[currentIndex], heapArray[targetIndex]] = [heapArray[targetIndex], heapArray[currentIndex]];
  ...
  }, [inputValue, heap, maxNodes, heapType, heapifyUp, shouldSwap, isOperating, onOperationComplete]);
  ...
  const renderEdges = () => {
    ...
    heap.forEach((node, index) => {
      ...
      if (leftChildIndex < heap.length) {
        ...
        if (leftPos) {
          const isHighlighted = highlightedNodes.includes(index) && highlightedNodes.includes(leftChildIndex);
          edges.push(
            <line
      ...
      if (rightChildIndex < heap.length) {
        ...
        if (rightPos) {
          const isHighlighted = highlightedNodes.includes(index) && highlightedNodes.includes(rightChildIndex);
          edges.push(
            <line
      ...
      <div className={`mb-4 p-4 rounded-lg ${
        ...
        <div className="flex items-center justify-between">
          <div className="text-sm">
            ...
            <span className={`font-semibold ml-2 ${isValidHeap() ? 'text-green-600' : 'text-red-600'}`}>
        ...
        {heap.length === 0 && (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-center text-gray-500">
              ...
              <p className="text-sm">Add nodes to build your {heapType === 'max' ? 'Max' : 'Min'} Heap</p>
